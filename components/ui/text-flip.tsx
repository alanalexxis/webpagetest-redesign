...
export function ContainerTextFlip({
  ...
  // Set up interval for word cycling
  useEffect(() => {
  ...
  }, [words.length, interval]); // Only depend on array length, not the array itself

  // Update width when the word changes
  useEffect(() => {
    updateWidthForWord();
  }, [currentWordIndex]);

  // Check for width updates on mount and resize
  useEffect(() => {
    const handleResize = () => {
      updateWidthForWord();
    };
    ...
    updateWidthForWord(); // Initial measurement
  ...
  return (
    <motion.div
      ...
      key={`container-${words[currentWordIndex]}`}
    >
      <motion.div
        transition={{
